...
class LeaderboardDailyFragment : BaseFragment() {
    ...
    private var arrayListLeaderboard = arrayListOf<LeaderBoard>()
    private var arrayListLeaderboardFinal = arrayListOf<LeaderBoard>()
...
//        Handler(Looper.getMainLooper()).postDelayed({
        homeViewModel.getLeaderboardDaily(page)
    }


    /*override fun onResume() {
        super.onResume()
        Logger.d("CALLING=> onResume " )
        arrayListLeaderboard.clear()
        arrayListLeaderboardFinal.clear()
        mAdapter?.notifyDataSetChanged()
//        showProgress()
        if (page == 1){
            mShimmerLeaderboard.visible()
            nestedScrollViewLeaderBoard.gone()
            llEmpty.gone()
        }
//        Handler(Looper.getMainLooper()).postDelayed({
...
//        },300)
    ...
    private fun observer() {
        homeViewModel.LeaderboardObserverDaily.observe(this, Observer {
            isInProgress = false
//            hideProgress()
