...
import com.bluboy.android.databinding.LayoutContentLeaderboardBinding
import com.bluboy.android.presentation.core.BaseFragment
import com.bluboy.android.presentation.game.adapters.LeaderBoardGamesAdapter
import com.bluboy.android.presentation.home.HomeViewModel
import com.bluboy.android.presentation.utility.*
import org.koin.android.viewmodel.ext.android.sharedViewModel


class LeaderboardGameFragment : BaseFragment() {

    private var gameId = ""
    private var mAdapter: LeaderBoardGamesAdapter? = null
    private val homeViewModel: HomeViewModel by sharedViewModel()
    private lateinit var leaderboardRs: LeaderboardRs
    private var page = 1
    private var isInProgress = false
    private var arrayListLeaderboard = ArrayList<LeaderBoard>()
    private var arrayListLeaderboardFinal = ArrayList<LeaderBoard>()
    private lateinit var binding: LayoutContentLeaderboardBinding
    ...
    override fun onCreateView(
        ...
        binding = LayoutContentLeaderboardBinding.inflate(inflater, container, false);
        return binding.root
    }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        Glide.with(requireContext()).asGif().load(R.raw.loader).into(binding.progress)
        setAdapter()
    ...
    private fun observer() {
        homeViewModel.GameLeaderboardObserver.observe(this, Observer {
            isInProgress = false
//            hideProgress()
...
//
    ...
    private fun setAdapter() {
        mAdapter = LeaderBoardGamesAdapter(requireContext(), arrayListLeaderboardFinal, {
        ...
        binding.nestedScrollViewLeaderBoard.setOnScrollChangeListener(NestedScrollView.OnScrollChangeListener { v, scrollX, scrollY, oldScrollX, oldScrollY ->
            if (scrollY == v.getChildAt(0).measuredHeight - v.measuredHeight) {
                ...
                if (leaderboardRs?.data?.isHaveMoreRecords.equals("Yes") && !isInProgress) {
                    ...
                    getLeaderBoards(gameId, true)
                }
            }
        })
    }

    fun getLeaderBoards(gameId: String, isScroll: Boolean? = false) {
        if (isInProgress) {
            return
        }
        isInProgress = true
...
//        Handler(Looper.getMainLooper()).postDelayed({
        homeViewModel.getGameLeaderboard(gameId, page)
//        },200)
    }
}