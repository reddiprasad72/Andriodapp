...
class LeaderboardMontlyFragment : BaseFragment() {
...
//        Handler(Looper.getMainLooper()).postDelayed({
        homeViewModel.getLeaderboardMonthly(page)
    }

    private fun observer() {
        homeViewModel.LeaderboardObserverMonthly.observe(this, Observer {
            isInProgress = false
//            hideProgress()
                ...
                if (it.data != null && arrayListLeaderboard.isNotEmpty()) {
                    ...
                    if (arrayListLeaderboard.size > 0) {
                        binding.topThree.apply {
                            imageViewProfileone.visible()
                            imageViewProfilebg.visible()
                            ivRankOneGold.visible()
                            imageCrown.visible()
                            textViewUserOneName.visible()
                            imageViewBackgroundUserOne.visible()
                            textViewUserOnePoint.visible()
                            textViewRankOne.gone()
                        }
                    ...
                    if (it.data?.ownRank != null && it.data?.ownRank?.userName != null) {
                        ...
                        binding.ownRankLayout.textViewUserName.text =
                            it.data?.ownRank?.userName.toString() + " (You)"
